...
The SC2-IngameDevTools project organises each feature as a self-contained **handler file** in
`Script/`: one main coordinator (`DevToolsMain.galaxy`) that includes every handler and calls
each `*_Init()`, a shared utilities folder (`DevTools/`), and `_h.galaxy` headers holding only
forward declarations. Each handler file follows the same anatomy: includes → UPPER_SNAKE_CASE
path constants → module struct/global → file-private `static` state → trigger event functions →
one public `_Init()` function (triggers may be registered by STRING name).

The complete folder tree, coordinator source, editor-lib entry wiring, header pattern, and
handler anatomy example live in
[references/handler-module-layout.md](references/handler-module-layout.md).
