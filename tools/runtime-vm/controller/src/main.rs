...
use windows_sys::Win32::Storage::FileSystem::{CreateFileW, ReadFile, WriteFile, FILE_ATTRIBUTE_NORMAL, FILE_GENERIC_READ, FILE_GENERIC_WRITE, OPEN_EXISTING};
...
fn json_string(source: &str, key: &str) -> Result<String, String> {
    ...
    let start = source.find(&marker).ok_or_else(|| format!("profile key missing: {key}"))?;
    let rest = &source[start + marker.len()..];
    let colon = rest.find(':').ok_or_else(|| format!("profile key malformed: {key}"))?;
    let value = rest[colon + 1..].trim_start();
    if !value.starts_with('"') {
        return Err(format!("profile key is not a string: {key}"));
    }
    let end = value[1..].find('"').ok_or_else(|| format!("profile string unterminated: {key}"))? + 1;
...
fn json_u32(source: &str, key: &str) -> Result<u32, String> {
    ...
    let rest = &source[start + marker.len()..];
    let colon = rest.find(':').ok_or_else(|| format!("profile key malformed: {key}"))?;
    rest[colon + 1..].trim_start().split(|ch: char| !ch.is_ascii_digit()).next().unwrap_or("").parse().map_err(|_| format!("profile key is not a number: {key}"))
}
...
fn json_bool(source: &str, key: &str) -> Result<bool, String> {
    ...
    let colon = rest.find(':').ok_or_else(|| format!("profile key malformed: {key}"))?;
    let value = rest[colon + 1..].trim_start();
    if value.starts_with("true") { Ok(true) } else if value.starts_with("false") { Ok(false) } else { Err(format!("profile key is not boolean: {key}")) }
}
...
fn usage() -> ! {
    eprintln!("usage: gsvm-controller inject --pid PID --dll DLL --profile PROFILE --ack-debug-process [--timeout-ms N]");
    std::process::exit(2)
}
...
fn query_image_path(process: HANDLE) -> Result<PathBuf, String> {
    ...
    let ok = unsafe {
        windows_sys::Win32::System::Threading::QueryFullProcessImageNameW(
            process, 0, buffer.as_mut_ptr(), &mut capacity,
        )
    };
    if ok == 0 {
        return Err(format!("QueryFullProcessImageNameW failed: {}", unsafe { GetLastError() }));
    }
    Ok(PathBuf::from(String::from_utf16_lossy(&buffer[..capacity as usize])))
}

fn inject(process: HANDLE, dll: &Path) -> Result<(), String> {
    let encoded = wide(dll.as_os_str());
    let bytes = encoded.len() * std::mem::size_of::<u16>();
    let remote = unsafe { VirtualAllocEx(process, null(), bytes, MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE) };
    if remote.is_null() {
    ...
    let write_ok = unsafe {
        windows_sys::Win32::System::Diagnostics::Debug::WriteProcessMemory(
            process, remote, encoded.as_ptr() as *const c_void, bytes, &mut written,
        )
    };
    ...
    if wait != WAIT_OBJECT_0 || exit_code == 0 {
        return Err(format!("LoadLibraryW did not complete successfully (wait={wait}, module={exit_code:#x})"));
    }
    Ok(())
}
...
fn open_pipe(name: &str, timeout: Duration) -> Result<HANDLE, String> {
    ...
    loop {
        ...
        if Instant::now() >= deadline {
            return Err(format!("named pipe unavailable: {}", unsafe { GetLastError() }));
        }
        unsafe { WaitNamedPipeW(encoded.as_ptr(), NMPWAIT_WAIT_FOREVER); }
...
fn pipe_request(pipe: HANDLE, command: &str) -> Result<String, String> {
    ...
    let ok = unsafe { WriteFile(pipe, bytes.as_ptr(), bytes.len() as u32, &mut written, null_mut()) };
    if ok == 0 {
        return Err(format!("pipe write failed: {}", unsafe { GetLastError() }));
    }
    let mut buffer = [0u8; 4096];
    let mut read = 0u32;
    let ok = unsafe { ReadFile(pipe, buffer.as_mut_ptr(), buffer.len() as u32, &mut read, null_mut()) };
    if ok == 0 {
        return Err(format!("pipe read failed: {}", unsafe { GetLastError() }));
    }
    Ok(String::from_utf8_lossy(&buffer[..read as usize]).trim().to_string())
}
...
fn run(args: &[String]) -> Result<(), String> {
    ...
    let pid: u32 = arg_value(args, "--pid").ok_or("--pid is required")?.parse().map_err(|_| "invalid --pid")?;
    ...
    let timeout = arg_value(args, "--timeout-ms").unwrap_or_else(|| "10000".to_string()).parse::<u64>().map_err(|_| "invalid --timeout-ms")?;
    let profile = parse_profile(&fs::read_to_string(&profile_path).map_err(|err| format!("profile read failed: {err}"))?)?;
    ...
    let access = PROCESS_CREATE_THREAD | PROCESS_QUERY_LIMITED_INFORMATION | PROCESS_VM_OPERATION | PROCESS_VM_READ | PROCESS_VM_WRITE;
    ...
    let result = (|| {
        ...
        let file_name = image.file_name().and_then(|name| name.to_str()).unwrap_or_default();
        ...
        unsafe { CloseHandle(pipe); }
        println!("{{\"pid\":{pid},\"image\":{},\"sha256\":{},\"hello\":{},\"status\":{},\"shutdown\":{}}}",
            json_quote(&image.display().to_string()), json_quote(&actual_hash), json_quote(&hello), json_quote(&status), json_quote(&shutdown));
        Ok(())
    })();
