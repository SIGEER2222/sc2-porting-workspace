...
<main class="shell">
  ...
  <section class="layout">
    ...
    <aside class="side">
      <section class="panel wide"><h2>当前状态</h2><div class="stats"><div class="stat"><span>地图</span><strong id="mapName"></strong></div><div class="stat"><span>Loop</span><strong id="loop"></strong></div><div class="stat"><span>游戏时间</span><strong id="gameTime"></strong></div><div class="stat"><span>状态版本</span><strong id="stateVersion"></strong></div><div class="stat"><span>Context 版本</span><strong id="contextVersion"></strong></div><div class="stat"><span>阶段</span><strong id="phase"></strong></div><div class="stat"><span>终局</span><strong id="endReason"></strong></div><div class="stat"><span>夜晚 / 波次</span><strong id="nightWave"></strong></div><div class="stat"><span>友军 / 敌军 / 中立</span><strong id="entityTotals"></strong></div><div class="stat"><span>地图 Objects</span><strong id="mapObjectCount"></strong></div></div></section>
...
const STATIC_TYPE_ALIASES = {"InfestedCocoon":"Bunker","InfestedMaw":"SpineCrawler","JarbanInfestibleColonistHut":"Bunker","InfestableHut":"Bunker","InfestableBiodome":"Bunker","InfestedMercHaven":"Barracks","CreepTumor":"SpineCrawler","CreepTumorUsed":"SpineCrawler","SpacePlatformGeyser":"VespeneGeyser"};
...
const mapImageSize = MAP_META.minimap_size || {width:256,height:256};
...
function sourceObjectKey(value) { return value === undefined || value === null ? null : String(value); }
function destroyedSourceObjectIds() { const result=new Set(); for(let i=0;i<FRAMES.length && i<=index;i++) for(const event of eventsOf(FRAMES[i])) if(event.kind === "infested_structure_destroyed" && sourceObjectKey(event.source_object_id)!==null) result.add(sourceObjectKey(event.source_object_id)); return result; }
function staticEntities() { const destroyed=destroyedSourceObjectIds(); return staticObjects.map(object => { const dead=destroyed.has(sourceObjectKey(object.source_object_id)); return normalize({entity_id:object.id, unit_type_id:object.unit_type_id, owner:object.owner, x:object.x, y:object.y, health:dead?0:100, shields:0, energy:0, state:dead?"destroyed":"map-static", alive:!dead, source_object_id:object.source_object_id, source_map_id:object.id, source_unit_type_id:object.source_unit_type_id}, Number(object.owner), false); }); }
...
const EMPTY_FRAME = {record_type:"frame",loop:0,events:[],context:{}};
function current() { return FRAMES[index] || FRAMES[0] || EMPTY_FRAME; }
...
function pos(x,y) {
  if (MAP_META.minimap_data_url) {
    ...
    return [((mapImageRect.x+nx*mapImageRect.w)/mapImageSize.width)*canvas.width, ((mapImageRect.y+ny*mapImageRect.h)/mapImageSize.height)*canvas.height];
  }
...
function drawStaticObject(object, destroyedIds) {
  const [x,y]=pos(object.x,object.y); const renderType=object.simulation_unit_type_id || STATIC_TYPE_ALIASES[object.unit_type_id] || object.unit_type_id; const r=radius(renderType)*0.78; const destroyed=destroyedIds.has(sourceObjectKey(object.source_object_id));
  ctx.save(); ctx.globalAlpha=destroyed?0.9:(object.owner===0?0.62:0.72); ctx.fillStyle=destroyed?"#5e646b":color(object.owner); ctx.strokeStyle=destroyed?"#ff6b6b":"#101820"; ctx.lineWidth=destroyed?2:1;
  if (BUILDINGS.has(renderType)) { ctx.fillRect(x-r,y-r,r*2,r*2); ctx.strokeRect(x-r,y-r,r*2,r*2); }
  else { ctx.beginPath();ctx.arc(x,y,Math.max(1.8,r*.68),0,Math.PI*2);ctx.fill(); }
  if (destroyed) { ctx.beginPath();ctx.moveTo(x-r,y-r);ctx.lineTo(x+r,y+r);ctx.moveTo(x+r,y-r);ctx.lineTo(x-r,y+r);ctx.stroke(); }
  ctx.restore();
}
...
function draw() { const f=current(), es=entitiesOf(f), destroyedIds=destroyedSourceObjectIds(); drawGrid(); if(document.getElementById("staticLayer").checked) for(const object of staticObjects) drawStaticObject(object,destroyedIds); for(const e of es.filter(e=>e.owner===0))drawFrameEntity(e); for(const e of es.filter(e=>e.owner!==0 && !isFriendly(e.owner)))drawFrameEntity(e); for(const e of es.filter(e=>isFriendly(e.owner)))drawFrameEntity(e); for(const ev of eventsOf(f)){const e=es.find(x=>String(x.id)===String(ev.entity_id));if(e){const [x,y]=pos(e.x,e.y);ctx.strokeStyle="#f4b860";ctx.lineWidth=3;ctx.beginPath();ctx.arc(x,y,radius(e.type)+7,0,Math.PI*2);ctx.stroke();}} document.getElementById("mapHud").textContent=`${mapNameOf(f)} · Loop ${f.loop} · ${formatTime(f.loop)} · Night ${f.context?.mission?.night ?? f.current_night ?? 0} · ${MAP_PROJECTION?"显示投影":"原始坐标"} · ${f.label || "frame"}`; updateSidebar(f,es); document.getElementById("seek").value=index; document.getElementById("timeCurrent").textContent=`Loop ${f.loop} · ${formatTime(f.loop)}`; }
...
const MARKER_KINDS = new Set(["night_started","night_attack_plan_armed","map_script_wave_spawned","special_infested_spawned","white_noise_infested_spawned","hybrid_reinforcements_spawned","map_wave_structure_targeted","building_reinforcements_spawned","daytime_infected_cleared","build_completed","train_completed","infested_structure_destroyed"]);
function setupMarkers() { const strip=document.getElementById("markers"),den=Math.max(1,FRAMES.length-1);ACTIONS.forEach(a=>{const m=document.createElement("span");m.className="marker action";m.title=`${a.name} @ loop ${a.loop}`;m.style.left=`${FRAMES.findIndex(f=>f.loop>=a.loop)/den*100}%`;m.onclick=()=>jumpToLoop(a.loop);strip.appendChild(m);});FRAMES.forEach((f,i)=>eventsOf(f).filter(ev=>MARKER_KINDS.has(ev.kind)).forEach(ev=>{const m=document.createElement("span");m.className="marker";m.title=`${ev.kind} @ loop ${ev.loop}`;m.style.left=`${i/den*100}%`;m.onclick=()=>setIndex(i);strip.appendChild(m);})); }
...
const firstFrame = FRAMES[0] || EMPTY_FRAME, lastFrame = FRAMES[FRAMES.length-1] || EMPTY_FRAME;
document.getElementById("headerMeta").textContent=`${SUMMARY.replay_id||"replay"} · ${SUMMARY.evidence_type||"simulator"}`;document.getElementById("footerMeta").textContent=`${SUMMARY.actions_successful??SUMMARY.actions_completed??0}/${SUMMARY.actions_total??0} actions · ${SUMMARY.event_count??0} events · ${allEntities.length} dynamic snapshots · ${staticObjects.length} map objects · trace ${String(SUMMARY.trace_sha256||"").slice(0,12)}`;document.getElementById("seek").max=Math.max(0,FRAMES.length-1);document.getElementById("timeStart").textContent=`Loop ${firstFrame.loop} · ${formatTime(firstFrame.loop)}`;document.getElementById("timeEnd").textContent=`Loop ${lastFrame.loop} · ${formatTime(lastFrame.loop)}`;const replayStatus=document.getElementById("replayStatus");replayStatus.textContent=IS_STATIC_PREVIEW?"静态地图预览 · 无动态回放帧":MAP_PROJECTION?`${FRAMES.length} frames · 可播放 · 真实地图底图 / simulator 显示投影`:`${FRAMES.length} frames · 可播放`;replayStatus.classList.toggle("static",IS_STATIC_PREVIEW);if(IS_STATIC_PREVIEW){document.querySelectorAll("#play,#back,#forward,#start,#end,#seek,.speed").forEach(control=>control.disabled=true);}setupMarkers();draw();
</script>
</body>
</html>
'''
