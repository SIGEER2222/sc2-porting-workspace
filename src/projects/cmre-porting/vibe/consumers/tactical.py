    ...
    def decide(self, obs: Observation, loop: int) -> list[TacticalAction]:
        ...
        for u in obs.own_units:
            actions.append(TacticalAction(
                entity_id=u["entity_id"], kind="attack",
                target_entity_id=target["entity_id"],
                reason=f"focus_fire lowest_hp={target['health']}",
                trace_refs={"target_entity_id": target["entity_id"],
...
class SpreadFireStrategy(Strategy):
    ...
    def decide(self, obs: Observation, loop: int) -> list[TacticalAction]:
        ...
        actions = []
        for u in obs.own_units:
            nearest = min(obs.visible_enemies, key=lambda e: (
                e["x"] - u["x"]) ** 2 + (e["y"] - u["y"]) ** 2)
            actions.append(TacticalAction(
                entity_id=u["entity_id"], kind="attack",
                target_entity_id=nearest["entity_id"],
                reason=f"spread_fire nearest dist={math.hypot(nearest['x']-u['x'], nearest['y']-u['y']):.2f}",
                trace_refs={"target_entity_id": nearest["entity_id"],
                            "target_dist": math.hypot(nearest["x"] - u["x"], nearest["y"] - u["y"])},
            ))
        return actions
    ...
    def decide(self, obs: Observation, loop: int) -> list[TacticalAction]:
        ...
        for u in obs.own_units:
            ...
            if dist < self.retreat_range:
                ...
                actions.append(TacticalAction(
                    entity_id=u["entity_id"], kind="move",
                    target_x=tx, target_y=ty,
                    reason=f"kite_retreat dist={dist:.2f}<{self.retreat_range}",
                    trace_refs={"nearest_enemy": nearest["entity_id"], "dist": dist,
                                "decision": "retreat"},
                ))
                ...
                actions.append(TacticalAction(
                    entity_id=u["entity_id"], kind="move",
                    target_x=nearest["x"], target_y=nearest["y"],
        ...
        for u in obs.own_units:
            ...
            if hp_ratio < self.hp_threshold_pct and obs.visible_enemies:
                ...
                actions.append(TacticalAction(
                    ...
                    trace_refs={"unit_hp": hp, "unit_max_hp": max_hp,
                ...
                actions.append(TacticalAction(
                    ...
                    trace_refs={"unit_hp": hp, "nearest_enemy": nearest["entity_id"],
                                "decision": "attack"},
                ))
        return actions
...
class HealAbilityStrategy(Strategy):
    ...
    def __init__(self, heal_threshold_pct: float = 0.7,
    ...
    def decide(self, obs: Observation, loop: int) -> list[TacticalAction]:
        ...
        healers = [u for u in obs.own_units
        ...
        for healer in healers:
            actions.append(TacticalAction(
                entity_id=healer["entity_id"], kind="ability",
                target_entity_id=target["entity_id"],
                ability_id=self.heal_ability_id,
                reason=(f"heal_ability target_hp={target['health']} "
...
def _run_single(scenario_dict: dict, strategy: Strategy, seed: int,
                ally_player_id: int = 1,
                max_loops: int = 1000) -> SingleRunMetrics:
    ...
    decisions_made = 0
    initial_friendlies = len([e for e in s.world.entities.values()
                              if e.owner_player_id == ally_player_id and e.is_alive])
    initial_enemies = len([e for e in s.world.entities.values()
                           if e.owner_player_id != ally_player_id and e.is_alive])
    # 跟踪总伤害
    initial_enemy_hp = sum(e.health.raw for e in s.world.entities.values()
                           if e.owner_player_id != ally_player_id and e.is_alive)
    ...
    while not s.terminated and s.world.clock.now.loop < max_loops:
        ...
        for a in last_actions:
            ...
            if a.kind == "attack" and a.target_entity_id:
                ...
                # 但命令下发与决策时机可验证。
                s.unit_order([a.entity_id], "cast_unit",
                             issuer_player_id=ally_player_id,
                             target_entity_id=a.target_entity_id,
    ...
    final_friendlies = [e for e in s.world.entities.values()
                        if e.owner_player_id == ally_player_id and e.is_alive]
    final_enemies = [e for e in s.world.entities.values()
                     if e.owner_player_id != ally_player_id and e.is_alive]
    ...
    return SingleRunMetrics(
        ...
        winner=winner,
        survivors={pid: sum(1 for e in final_friendlies if e.owner_player_id == pid)
                   for pid in {1, 2}},
...
def _aggregate(strategy: Strategy, scenario_dict: dict, seeds: list[int],
               ally_player_id: int = 1, max_loops: int = 1000) -> AggregatedMetrics:
    runs = [_run_single(scenario_dict, strategy, seed, ally_player_id, max_loops) for seed in seeds]
...
def run_tactical_ab(
    ...
    if agg_b.win_rate > agg_a.win_rate:
        improvements.append(f"{agg_b.strategy_name} win_rate {agg_b.win_rate:.2f} > {agg_a.strategy_name} {agg_a.win_rate:.2f}")
        trace_refs.append({"metric": "win_rate", "a": agg_a.win_rate, "b": agg_b.win_rate,
    ...
    if agg_b.avg_exchange_ratio > agg_a.avg_exchange_ratio:
        improvements.append(f"{agg_b.strategy_name} exchange_ratio {agg_b.avg_exchange_ratio:.2f} > {agg_a.strategy_name} {agg_a.avg_exchange_ratio:.2f}")
        trace_refs.append({"metric": "exchange_ratio",
                           "a_per_seed": [r.enemies_killed / max(1, r.friendlies_lost) for r in agg_a.seed_results],
                           "b_per_seed": [r.enemies_killed / max(1, r.friendlies_lost) for r in agg_b.seed_results]})

    improvement_claim = "; ".join(improvements) if improvements else "无明显改进"
    verdict = "PASS" if confidence in ("high", "medium") and improvements else "INCONCLUSIVE"
...
def p4c_selftest() -> dict:
    ...
    report = run_tactical_ab(scenario_dict, strat_a, strat_b, seeds,
                             ally_player_id=1, max_loops=400)

    # 1) 两策略都跑了
    checks["both_strategies_ran"] = (
        report.strategy_a.seed_count == len(seeds) and report.strategy_b.seed_count == len(seeds)
    )
    ...
    checks["multi_seed_metrics"] = (
        report.strategy_a.seed_count > 1 and report.strategy_b.seed_count > 1
    ...
    details["traces_produced"] = f"a_hashes[0]={a_hashes[0][:12] if a_hashes else 'none'}"

    # 6) 战斗确实发生（enemies_killed > 0 至少一个 seed）
    any_kills = any(r.enemies_killed > 0 for r in report.strategy_a.seed_results + report.strategy_b.seed_results)
    ...
    obs_near = Observation(
        loop=0, player_id=1,
        own_units=[{"entity_id": 1, "unit_type_id": "Marine", "owner": 1,
                    "x": 0.0, "y": 0.0, "health": 45, "shields": 0, "energy": 0,
                    "state": "idle", "max_health": 45}],
        visible_enemies=[{"entity_id": 2, "unit_type_id": "Zergling", "owner": 2,
                          "x": 2.0, "y": 0.0, "health": 35, "shields": 0, "energy": 0,
                          "state": "idle", "max_health": 35}],
        resources={}, mission={},
    )
    act_near = kite_strat.decide(obs_near, 0)
    checks["positioning_retreat_when_too_close"] = (
        len(act_near) == 1 and act_near[0].kind == "move"
    ...
    obs_mid = Observation(
        ...
        own_units=[{"entity_id": 1, "unit_type_id": "Marine", "owner": 1,
                    ...
                    "state": "idle", "max_health": 45}],
        visible_enemies=[{"entity_id": 2, "unit_type_id": "Zergling", "owner": 2,
                          "x": 4.0, "y": 0.0, "health": 35, "shields": 0, "energy": 0,
                          "state": "idle", "max_health": 35}],
        resources={}, mission={},
    )
    act_mid = kite_strat.decide(obs_mid, 0)
    checks["positioning_attack_when_in_range"] = (
        len(act_mid) == 1 and act_mid[0].kind == "attack"
    ...
    obs_far = Observation(
        ...
        own_units=[{"entity_id": 1, "unit_type_id": "Marine", "owner": 1,
                    ...
                    "state": "idle", "max_health": 45}],
        visible_enemies=[{"entity_id": 2, "unit_type_id": "Zergling", "owner": 2,
                          "x": 8.0, "y": 0.0, "health": 35, "shields": 0, "energy": 0,
                          "state": "idle", "max_health": 35}],
        resources={}, mission={},
    )
    act_far = kite_strat.decide(obs_far, 0)
    checks["positioning_approach_when_too_far"] = (
        len(act_far) == 1 and act_far[0].kind == "move"
    ...
    obs_retreat = Observation(
        ...
        own_units=[
            {"entity_id": 1, "unit_type_id": "Marine", "owner": 1,
             "x": 0.0, "y": 0.0, "health": 20, "shields": 0, "energy": 0,
             "state": "idle", "max_health": 45},  # 20/45 = 0.44 < 0.5 -> retreat
            {"entity_id": 3, "unit_type_id": "Marine", "owner": 1,
             "x": 5.0, "y": 5.0, "health": 45, "shields": 0, "energy": 0,
             "state": "idle", "max_health": 45},  # 45/45 = 1.0 >= 0.5 -> attack
        ],
        visible_enemies=[{"entity_id": 2, "unit_type_id": "Zergling", "owner": 2,
                          "x": 3.0, "y": 0.0, "health": 35, "shields": 0, "energy": 0,
                          "state": "idle", "max_health": 35}],
        resources={}, mission={},
    )
    ...
    checks["retreat_low_hp_moves_away"] = (
        low_hp_act is not None and low_hp_act.kind == "move"
    ...
    checks["retreat_full_hp_attacks"] = (
        full_hp_act is not None and full_hp_act.kind == "attack"
    ...
    obs_heal = Observation(
        loop=0, player_id=1,
        own_units=[
            {"entity_id": 10, "unit_type_id": "Medivac", "owner": 1,
             "x": 0.0, "y": 0.0, "health": 150, "shields": 0, "energy": 50,
             "state": "idle", "max_health": 150},  # 治疗者
            {"entity_id": 11, "unit_type_id": "Marine", "owner": 1,
             "x": 1.0, "y": 0.0, "health": 30, "shields": 0, "energy": 0,
             "state": "idle", "max_health": 45},  # 30/45 = 0.67 < 0.7 -> 受伤
        ],
        visible_enemies=[],
        resources={}, mission={},
    )
    ...
    checks["ability_timing_heal_when_wounded"] = (
        len(act_heal) == 1 and act_heal[0].kind == "ability"
    ...
    obs_full = Observation(
        ...
        own_units=[
            {"entity_id": 10, "unit_type_id": "Medivac", "owner": 1,
             "x": 0.0, "y": 0.0, "health": 150, "shields": 0, "energy": 50,
             "state": "idle", "max_health": 150},
            {"entity_id": 11, "unit_type_id": "Marine", "owner": 1,
             "x": 1.0, "y": 0.0, "health": 45, "shields": 0, "energy": 0,
             "state": "idle", "max_health": 45},  # 45/45 = 1.0 >= 0.7
        ],
        visible_enemies=[],
        resources={}, mission={},
    )
    ...
    obs_no_healer = Observation(
        ...
        own_units=[
            {"entity_id": 11, "unit_type_id": "Marine", "owner": 1,
             "x": 1.0, "y": 0.0, "health": 10, "shields": 0, "energy": 0,
             "state": "idle", "max_health": 45},  # 受伤但无 Medivac
        ],
        visible_enemies=[],
        resources={}, mission={},
    )
    ...
    kite_report = run_tactical_ab(scenario_dict, kite_strat, strat_a,
                                  seeds=[42, 43], ally_player_id=1, max_loops=300)
