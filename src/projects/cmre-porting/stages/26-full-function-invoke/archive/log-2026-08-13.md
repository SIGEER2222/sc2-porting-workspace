...
### 2026-08-13 WebUI map preview images PASS

- `static`：检查三个地图源后确认 66 张地图均有真实图片资产：CMRE 15 张使用已有
  Battle.net/加载预览图；虫心 20 张和起义狂潮 31 张使用各自地图目录内的
  `Minimap.tga`。外部地图源保持只读，不复制或修改源地图。
- `static`：`tools/cmre-webui/server.py` 为虫心/起义狂潮增加虚拟
  `MapPreview/<package>/<map>/Minimap.tga` 资源路径，资源路由仅接受登记的地图类别、地图目录
  和固定文件名，避免把本机绝对绑定路径暴露给浏览器。TGA 转 PNG 缓存写入
  `artifacts/projects/cmre-porting/stage26-full-function-invoke/runtime/webui-map-preview-cache/`，
  本轮生成 51 个缓存文件。
- `static`：`python -m pytest -q tools/cmre-webui/test_launch_async_contract.py` -> 22 passed；
  新增回归确认 66 张地图全部有 preview、源文件可解析、转换结果是 PNG；
  `py_compile`、`node --check`、`git diff --check` 通过。
- `runtime/browser`：重启用户指定的 `http://127.0.0.1:8767/` 后，真实 Chromium 点击地图标签并
  滚动触发全部 lazy 图片加载；66/66 张图片 `complete=true` 且 `naturalWidth>0`，占位块 0，
  浏览器错误 0。虫心/起义狂潮的 Minimap.tga 均实际转码并显示；截图和逐卡片 DOM 结果保存于
  `artifacts/projects/cmre-porting/stage26-full-function-invoke/runtime/webui-map-preview-20260813/`。

### 2026-08-13 Reborn Abathur Larva card parity gate

- `static`：新增 `generate_reborn_abathur_baseline.py`，从原版 Reborn `Larva` 的
  `CardLayouts` 生成唯一命令基线，并从 `CAbilTrain/InfoArray` 解析精确产物与数量。合并
  Swarm campaign 继承层后，`LarvaTrainSwarm,Train4 -> Devourer` 被保留；总计 28 个
  卡牌命令，`Blightbringer/Train27` 未在 Larva 卡牌中，因此没有被伪造为要求项。基线文件还
  记录了源文件 SHA-256 和 repo-relative 输入路径：
  `artifacts/projects/cmre-porting/stage26-full-function-invoke/reborn-abathur-baseline.json`。
- `static`：基线把 Catalog `Structure` 属性与明确的基础 Zerg 建筑集分开处理；`LurkerDen`、
  `Hatchery`、`SpawningPool` 均分类为建筑。`reborn_abathur_check.py` 对缺 Larva、缺期望
  ability、未执行、action 拒绝、错/漏/额外产物和缺 P1/P2 census 一律失败关闭；census 只
  分类已观察 roster，不把开局未出现的后期科技误报为缺失。
- `runtime/inconclusive`：首次 `005800` 运行未启用 `RequestDebug(tech_tree)`，因此原版
  需求尚未满足，不能用于判断 card parity；原始产物
  `reborn-abathur-larva-runtime-20260813-005800-final.json` 仅保留为前置条件诊断。
- `runtime`：修正探针后用 approved launcher 在端口 `5098` 完成 Reborn mod staging、
  Abathur preset、packed map 的 CreateGame/JoinGame，并先执行
  `RequestDebug(all_resources, tech_tree, fast_build)`。P1/P2 raw census 分别为
  `Drone=24,Hatchery=2,Larva=6,Overlord=1,CoopCasterAbathur=1,ACHeroSpawnPlacement=1`
  与 `Drone=12,Hatchery=2,Larva=6,Overlord=1,ACHeroSpawnPlacement=1`；原版单位和建筑
  与地图/指挥官支持对象分开记录。
- `runtime`：有效运行在 game loop `19..974` 进行 79 次 readiness sample，P1 Larva
  `6..17`，最终仅公开 `LarvaTrain,2`（ability id 1344），与 28 个原版 card command
  交集为 0。controlled Larva 的 28 次查询均不可用、均未执行 action；比较器报告
  `EXPECTED_ABILITY_MISSING=28`（LarvaTrain 4、LarvaTrainSwarm 3、LarvaTrainSwarm2 21），
  状态 `FAIL`。同窗口没有新增非空 `*ScriptError*.txt`，因此这是实际 ability/card parity
  回归而非 map load 或脚本崩溃。
- `runtime evidence`：
...
- `static validation`：`python -m pytest -q
  src/projects/cmre-porting/stages/26-full-function-invoke/test_reborn_abathur_check.py` -> 9 passed；
  三份新增脚本 `python -m py_compile` 通过。adapter/Catalog 修复不在当前阶段写入范围；在恢复
  原版 Larva abilities、card command 和 exact products 前，不能把这个 gate 标为 PASS。
- `static validation`：重建基线时发现两个本地 `0.71` Reborn 解包源的 `UnitData.xml`、
  `AbilData.xml` 和 `Lib48DF4533.galaxy` SHA-256 不同；默认发现因而 fail-closed 拒绝猜选。
  使用已归档基线的三份 SHA-256 显式定位到匹配源并执行
  `generate_reborn_abathur_baseline.py --reborn-root <matched-source>`，稳定得到 28 条命令
  （`LarvaTrain=4`、`LarvaTrainSwarm=3`、`LarvaTrainSwarm2=21`）。随后 9 个测试、三脚本
  `py_compile`、JSON parse 和 `git diff --check` 均通过。
- `verification`：将 `011000` 真机 artifact 重新送入当前 `reborn_abathur_check.py`，结果为
  `FAIL`、`failure_count=28`、`checked_command_count=28`；P1 classification 仍为原版单位
  `Drone/Larva/Overlord` 与建筑 `Hatchery`。验证结束后只终止本次 launcher 所有的
  `SC2_x64.exe` PID 30372，launcher PID 30268 和 API `5098` 均已释放。
